#!/usr/bin/env node
// NOTE: Renamed to build.cjs at runtime — run via: node scripts/build.cjs
// Windows-friendly equivalent of build.sh — no bash required.
// Usage: node scripts/build.js [--checkout <path>]
...
// --- Find tsc ---
const TSC = path.join(CHECKOUT, 'node_modules', '.bin', 'tsc.cmd');
const TSC_SH = path.join(CHECKOUT, 'node_modules', '.bin', 'tsc');
const tscBin = fs.existsSync(TSC) ? TSC : (fs.existsSync(TSC_SH) ? TSC_SH : '');
if (!tscBin) {
...
// Remove @standard-schema if it exists as a real dir
...
if (fs.existsSync(pnpmDir)) {
  const entries = fs.readdirSync(pnpmDir).filter(e => /^@standard-schema\+spec@/i.test(e));
  if (entries.length > 0) {
