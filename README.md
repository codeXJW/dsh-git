...
| 提交历史浏览 | `git_log` | ✅ Git Graph 式泳道图（彩色分支线/merge 曲线/分支标签） |
...
- **历史页**：**Git Graph 式泳道图**（对齐 VSCode Git Graph 扩展）——左侧彩色分支线随泳道分叉/汇合（merge 为分叉曲线），HEAD 提交圆点加圈强调；右侧每行是「提交主题 + 分支标签（HEAD 绿 / 远程蓝 / tag 琥珀）+ 短 hash · 作者 · 时间」→ **点提交展开变更文件清单**（含 shortstat）→ **点文件查看该提交里此文件的 diff**（自动切到差异页）
